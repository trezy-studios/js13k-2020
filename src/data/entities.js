...
export let entities = {
	...
	robot(context, e) {
		...
		if (!isVictory && timeRemaining <= 0) {
			if (!robot_state.path || robot_state.path.length === 0 && e.x === robot_state.next.x && robot_state.next.y === e.y) {
				let potential = map.path({ x: e.x, y: e.y }, { x: -1, y: -1 });
				let end = entities.find(ent => ent.type === "exit");
				...
				robot_state.path = map.path({ x: e.x, y: e.y }, spot);
		...
		let x = TILE_SIZE.w * e.x
		let y = TILE_SIZE.h * e.y
		let sourceX = 8 + (Math.floor((frame % 60) / (60 / frames)) * 8)
